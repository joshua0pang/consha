...
ensures ParseAddRec(s, n).Some? ==> LocsE(ParseAddRec(s, n).val.0) == {};
{
  if n < 2 then None else (
    var t := SkipWS(Or(Or(Or(Or(Or(ParseDeref(s, n - 1),
                                  ParseAlloc(s, n - 1)),
                                ParseShare(s, n - 1)),
                             ParseCopy(s, n - 1)),
                          ParseVal(s)),
  ...
  match expr {
    ...
    case Var(x) => (
      ...
      if MoveType(TypeSigma(sig)[x]) then (
        ...
        assert HeapDeclarationsE(h, sig, Var(x));
    ...
    case Deref(re) => (
      if !re.V? then (
        ...
        assert HeapDeclarationsE(h, sig, V(h[re.val.sl]));
        (h, sig, V(h[re.val.sl]))
      )
    )
    ...
    case Share(se) =>
      if !se.V? then (
        ...
        var (h2, sig2, se2) := EvalE(h, sig, se);
        assert g == TypeCheckE(TypeSigma(sig2), Share(se2));
        assert HeapDeclarationsE(h2, sig2, Share(se2));
        (h2, sig2, Share(se2))
      ) else (
        assert se.val.Ref?;
        assert HeapDeclarationsE(h, sig, Share(se));
        assert {se.val.l} == LocsE(Share(se));
        assert se.val.l in h;
        assert {se.val.l} == LocsE(V(SRef(se.val.l)));
        assert HeapDeclarationsE(h, sig, V(SRef(se.val.l)));
        (h, sig, V(SRef(se.val.l)))
      )

    case Copy(ce) => (
      assert g == TypeCheckE(TypeSigma(sig), Alloc(Deref(ce)));
      assert HeapDeclarationsE(h, sig, Alloc(Deref(ce)));
      (h, sig, Alloc(Deref(ce)))
    )
